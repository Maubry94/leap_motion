...
import groundColorImageSource from "./assets/textures/ground.jpg";
import tntTextureImageSource from "./assets/textures/tnt.jpg";
import ironCubeColorImageSource from "./assets/textures/ironCube.png";
import goldCubeColorImageSource from "./assets/textures/goldCube.png";
import diamondCubeColorImageSource from "./assets/textures/diamondCube.png";
import wallColorTextureSource from "./assets/textures/wall.jpg";
...
// Create the texture
const groundColorTexture = textureLoader.load(groundColorImageSource);
...
const wallColorTexture = textureLoader.load(wallColorTextureSource);
wallColorTexture.wrapS = THREE.RepeatWrapping;
wallColorTexture.wrapT = THREE.RepeatWrapping;
wallColorTexture.repeat.x = 10;
wallColorTexture.repeat.y = 10;

const tntTexture = textureLoader.load(tntTextureImageSource);
const ironCubeColorTexture = textureLoader.load(ironCubeColorImageSource);
const goldCubeColorTexture = textureLoader.load(goldCubeColorImageSource);
const diamondCubeColorTexture = textureLoader.load(diamondCubeColorImageSource);
...
// 'Points' de la main 1
for (let i = 0; i < 5; i++) {
  const ball_1 = new THREE.Mesh(geometry, new THREE.MeshLambertMaterial({ color: 0x00ff00 }));
  tips.push(ball_1);
  const ball_2 = new THREE.Mesh(geometry, material);
  dips.push(ball_2);
  const ball_3 = new THREE.Mesh(geometry, material);
  pips.push(ball_3);
  const ball_4 = new THREE.Mesh(geometry, material);
  mcps.push(ball_4);
  const ball_5 = new THREE.Mesh(geometry, material);
  carps.push(ball_5);
}
...
for (let i = 0; i < 5; i++) {
  const ball_1 = new THREE.Mesh(geometry, new THREE.MeshLambertMaterial({ color: 0x00ff00 }));
  tips.push(ball_1);
  const ball_2 = new THREE.Mesh(geometry, material2);
  dips.push(ball_2);
  const ball_3 = new THREE.Mesh(geometry, material2);
  pips.push(ball_3);
  const ball_4 = new THREE.Mesh(geometry, material2);
  mcps.push(ball_4);
  const ball_5 = new THREE.Mesh(geometry, material2);
  carps.push(ball_5);
}
...
for (let i = 0; i < 10; i++) {
  const cube_2 = new THREE.Mesh(
    new THREE.BoxGeometry(2, 2, 2, 20, 1),
  ...
  const loop = () => {
    ...
    if (frame && frame.hands && frame.hands.length > 0) {
      ...
      if (time_grab_hand1 < 0.05) {
        ...
        if (hand1.palmNormal[1] < 0) {
          // Si la main est orienté en haut
          axis = new CANNON.Vec3(1, 0, 0);
          angle = -(Math.PI / 2) * -hand1.palmNormal[0];
        } else {
          // Si la main est orienté en bas
          axis = new CANNON.Vec3(1, 0, 0);
          angle = (Math.PI / 2) * -hand1.palmNormal[0];
        }
        ...
        for (let i = 0; i < frame.gestures.length; i++) {
          ...
          if (circleCount > 9 && active_circle == false) {
            ...
            for (let i = 0; i < bodies.length; i++) {
              if (i == 10) {
                bodies[i].position.x = -15;
                bodies[i].position.y = 16;
                bodies[i].position.z = 0;
              } else if (i >= 10) {
                bodies[i].position.x = -15;
                bodies[i].position.y = 14;
                bodies[i].position.z = 4 + 21 - (i - 1) * 2.5;
              } else if (i >= 8) {
                bodies[i].position.x = -15;
                bodies[i].position.y = 10;
                bodies[i].position.z = 4 + 15 - (i - 1) * 2.5;
              } else if (i >= 5) {
                bodies[i].position.x = -15;
                bodies[i].position.y = 6;
                bodies[i].position.z = 4 + 9 - (i - 1) * 2.5;
              } else if (i >= 1) {
                bodies[i].position.x = -15;
                bodies[i].position.y = 2;
                bodies[i].position.z = 4 - (i - 1) * 2.5;
              } else if (i == 0) {
                bodies[i].position.x = 3;
                bodies[i].position.y = 10;
                bodies[i].position.z = 0;
              }
      ...
      if (frame.gestures.length != 0) {
        ...
        for (let i = 0; i < frame.gestures.length; i++) {
          ...
          if (swipeCount > 8 && active_swipe == false) {
            active_swipe = true;
            console.log("swipe");
      ...
      if (frame.hands.length > 1) {
        // Si il y a les deux mains
        if (
          hand1.pinchStrength > 0.6 && // Si le PinchStrength de la main 1 est supérieur à 0.8
